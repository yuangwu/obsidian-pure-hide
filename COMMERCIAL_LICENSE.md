...
### [v 1.1.0] - 2026-08-07 — 许可证变更与代码架构重构
...
- **代码架构优化**：移除 `main.js` 中的 `injectStyles()` 动态样式注入方法，弹窗样式（许可证信息 + 打赏弹窗）全部迁移至 `styles.css` 集中管理。
  - 样式与逻辑解耦，符合传统 Obsidian 插件开发规范；
  - 便于用户自定义样式，提升可维护性；
  - 减少 JS 文件体积，略微提升加载效率。

- **版权声明统一**：所有源代码文件（`main.js`、`styles.css`）头部注释从专有声明更新为 AGPL-3.0 标准版权声明，保留“星号线框”视觉风格，统一作者、版本、兼容性等元信息格式。
