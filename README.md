...
打开 **设置 → 插件选项 → Pure Hide**，设置项按**使用场景**分组：顶部 Tab 提供 **全部 / 编辑体验 / 阅读体验 / 界面外观 / 系统** 五个视图，每个场景下按功能子组折叠展示（子组标题显示项数），可通过工具栏"展开全部 / 折叠全部"切换。每个选项都配有清晰的描述，数值型参数支持滑块调节（**滑块与数值框实时同步**），每项右侧有**恢复默认**按钮。**搜索框支持中文输入**（含拼音组合防抖），搜索命中时自动展开对应子组；若当前场景无命中，会自动切换到含结果的其他场景。
